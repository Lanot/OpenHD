...
class EthernetLink : public OHDLink {
 ...
 private:
  ...
  int VIDEO_PORT = 5910;
  int VIDEO_PORT_SECONDARY = 5911;
  int TELEMETRY_PORT = 5920;

  std::unique_ptr<openhd::UDPForwarder> m_video_tx;  // Video transmitter
  std::unique_ptr<openhd::UDPForwarder> m_video_tx_secondary;  // Video transmitter Secondary

  std::unique_ptr<openhd::UDPReceiver> m_video_rx;   // Video receiver
  std::unique_ptr<openhd::UDPReceiver> m_video_rx_secondary;   // Video receiver Secondary
